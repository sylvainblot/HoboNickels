...
Value getworkex(CWallet* pWallet, const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "SwissCoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "SwissCoin is downloading blocks...");
...
Value getwork(CWallet* pWallet, const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "SwissCoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "SwissCoin is downloading blocks...");
